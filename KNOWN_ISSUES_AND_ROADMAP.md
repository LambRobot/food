...
4. **Better ingredient matching.** ✅ **DONE** — `scripts/match_worklist.py` ranks unmatched/low-confidence/suspicious matches by **calorie impact**; drove ~40 curated overrides and structural matcher fixes: prefix-based override lookup (tolerates trailing junk like "chicken broth **divided**"), comma-clause selection ("boneless, skinless chicken thighs" → "chicken thighs", not "boneless"), token de-dup, and fixes for the highest-impact errors (wine→grain, beef/lamb/chicken→liver, turkey→giblets, mixed-veg→palm-oil, sweet-potato→leaves). **Measured:** calibration energy error 24% → **21.9%**, and the medium-confidence tier improved 38% → **23%**.
5. **Cooked/drained/raw normalization.** ✅ **DONE (core)** — canned/cooked legumes map to cooked USDA forms (correct drained density), grains/rice to dry forms; and **dried** legumes measured dry but matched to a cooked form get a ~2.7× density correction (fixes the 3× undercount, e.g. chili's dried kidney beans). Remaining nuance: ambiguous bare "lentils/beans" default to cooked.
