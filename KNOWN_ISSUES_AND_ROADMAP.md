...
1. **Golden-set regression tests.** Hand-curate expected values for ~25–40 representative recipes (macros within a tolerance, key matches, servings). Run on every engine change so fixes can't silently regress. *Cheap, permanent safety net.*
2. **Ground-truth calibration.** Compare ~20 recipes against a reference (USDA "What's in the Foods You Eat" tool, Edamam free tier, or hand calculation). Quantify the real error band and **calibrate the confidence score** against it. *Turns "±10–25%, asserted" into "measured".*
3. **Servings normalization pass.** The #1 per-serving error source. Build a dedicated servings resolver: detect container/yield phrasing, cross-check against total grams (e.g. a "4-serving" dish totaling 8 kg is wrong), and flag/repair outliers. Optionally infer servings from total cooked weight (~400–500 g/serving main).
