...
*Grounded in `nutrition_methodology_wiki.md`. Status: **implemented — all four scores live.** Decisions taken: data source = **USDA bulk download** (SR Legacy + Foundation, extracted to a committed compact reference); scores = **all four** (Nutri-Score, NRF9.3, NOVA, and HEI-2020 all computed); scope = **full** (macros + micronutrients). Cooking **nutrient-retention factors are now applied** (representative USDA Release-6 values by cooking method, detected from the directions) — they reduce heat-sensitive vitamins/minerals; macros are left conserved. HEI-2020 is an approximation (gram→cup/oz-equivalent proxies; added sugar ≈ total sugar) since exact HEI needs the USDA FPED cup-equivalent database.*
